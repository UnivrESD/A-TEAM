...
AteamMiner::AteamMiner(XmlNode *xmlNode)
    : PropertyMiner(xmlNode),
      _fchecker(nullptr),
      _antec2CandInProps(),
      _antec2CandOutProps(),
      _antec2CandInoutProps(),
      _templates(),
	...
	*/
	...
	if (constraints != nullptr) {
		...
		string maxNextDepth =
		    getAttributeValue(constraints, "maxNextDepth");
		messageErrorIf(maxVariables.empty(),
			       "The attribute maxVariables in the "
			       "xml tag constraints has "
			       "not been found");
		messageErrorIf(maxNextDepth.empty(),
			       "The attribute maxNextDepth in the "
			       "xml tag constraints has "
			       "not been found");

		_maxVariablesAntecedent =
		    static_cast<size_t>(std::stoi(maxVariables));
		_maxNextDepth = static_cast<size_t>(std::stoi(maxNextDepth));
	}
	//--------------------------------------------------------------------------
...
AteamMiner::~AteamMiner() {
	for (std::pair<const Template, Prop2DProps> &a : _antec2CandInProps) {
		for (auto &b : a.second) {
			for (auto &c : b.second) {
				delete c.second[0];
				delete c.second[1];
...
//	List<Proposition> illegalPropositions;
	//================ step 3: mining temporal assertions
	//======================
	List<Assertion> minedAssertions;
	for (Proposition *prop : cone.outPropositions) {
		...
		*/
		...
		// check for illegal propositions
		/*
		for (auto e : illegalPropositions) {
			cone.inPropositions.push_back(
			    illegalPropositions.pop_back());
		}
		assert(illegalPropositions.empty());
		auto it = cone.inPropositions.begin();
		while (it != cone.inPropositions.end()) {
			std::cout << oden::prop2String(**it) << ":"
				  << oden::prop2depth(**it) << "\n";
			std::cout << oden::prop2String(*prop) << ":"
				  << oden::prop2depth(*prop) << "\n";
			if (oden::prop2depth(*prop) < oden::prop2depth(**it)) {
				std::cout << "Removing"
					  << ":"
					  << oden::prop2String(
						 *cone.inPropositions.back())
					  << "\n";
				illegalPropositions.push_back(*it);
				*it = cone.inPropositions.pop_back();
			} else {
				it++;
			}
		}
		for (auto i : cone.inPropositions) {
			std::cout << "^^" << oden::prop2String(*i) << "\n";
		}
		*/
		...
		check_coverage:
			...
			while (!minedAssertions.empty()) {
				...
				if (oden::prop2depth(*assertion->antecedent) <=
				    oden::prop2depth(*assertion->consequent)) {
					assertion->id = assertCounter++;
					cone.assertions.push_back(assertion);
					ContingencyTableAnalysis::instance().add_assertion(assertion);
				}
...
bool AteamMiner::_makeImply(const Template &templ, Proposition *prop,
  ...
  */
	...
	// generator
	size_t l = prop->getMaxTime();
	std::cout << "getMaxTime: " << l << std::endl;
	traceInfo.initTraceInfo(l);
...
void AteamMiner::_finalizeAntecedent(Assertion &assertion,
	...
	if (props.size() > 1) {
		PropositionAnd *fAnt = new PropositionAnd();
		for (Proposition *proposition : props) {
			fAnt->addItem(oden::copy(*proposition));
		}
		assertion.antecedent = fAnt;
	} else {
		assertion.antecedent = oden::copy(**begin(props));
	}
	...
	if (antecedent.is(spot::op::ap)) {
	  ...
	  for (auto at : antecedent) {
	    ...
	    assertion.t2p[at] = (p == nullptr) ? new BooleanConstant(true, 0) :
	p; assertion.antecedent = (p == nullptr) ? new BooleanConstant(true, 0)
	: p;
	  }
	}
	*/
...
void AteamMiner::_makeCandidatePropositions(ConeOfInfluence &cone) {
	...
	for (const Template &temp : _templates) {
		...
		/*
	    std::sort(begin(cone.inPropositions),end(cone.inPropositions),[](Proposition
	    *p1,Proposition *p2){ return oden::prop2String(*p1) <
	    oden::prop2String(*p2);
		    });
		    */
		std::vector<Proposition *> nextProps;
		for (auto i : cone.inPropositions) {
			nextProps.push_back(i);
		}
		for (size_t i = 1; i <= _maxNextDepth; i++) {
			for (auto e : nextProps) {
				cone.inPropositions.push_back(
				    new PropositionNext(oden::copy(*e), i));
			}
		}
		...
		_makeCandidatePropositions(cone.inPropositions, chainOfNexts,
...
void AteamMiner::_makeCandidatePropositions(
	...
	if (!candidatePropositions.empty()) return;
	...
	for (Proposition *p : propositions) {
		...
		for (; rit != chainOfNexts.rend(); ++rit) {
			...
			dProp.second[1] =
			    makeExpression<PropositionNot>(oden::copy(*p));
