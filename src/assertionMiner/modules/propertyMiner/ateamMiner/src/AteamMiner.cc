...
AteamMiner::AteamMiner(XmlNode *xmlNode)
  ...
  */
  ...
  if (constraints != nullptr) {
    ...
    string maxNextDepth = getAttributeValue(constraints, "maxNextDepth");
    messageErrorIf(maxVariables.empty(), "The attribute maxVariables in the "
                                         "xml tag constraints has "
                                         "not been found");
    messageErrorIf(maxNextDepth.empty(), "The attribute maxNextDepth in the "
                                         "xml tag constraints has "
                                         "not been found");
...
void AteamMiner::_readTemplate(std::string formula) {
  ...
  messageErrorIf(!(antecedent.is(spot::op::ap) || antecedent.is(spot::op::And)),
...
void AteamMiner::_readAssertionTemplates(XmlNode *templatesXml) {
  ...
  messageErrorIf(templateList.empty(), "No assertion template has been found!");
...
void AteamMiner::_finalizeAntecedent(Assertion &assertion,
  ...
  if (antecedent.is(spot::op::ap)) {
    ...
    for (auto at : antecedent) {
      ...
      assertion.t2p[at] = (p == nullptr) ? new BooleanConstant(true, 0) : p;
      assertion.antecedent = (p == nullptr) ? new BooleanConstant(true, 0) : p;
    }
  }
  */
...
*p1,Proposition *p2){ return oden::prop2String(*p1) < oden::prop2String(*p2);
        });
        */
    ...
    for (size_t i = 1; i <= _maxNextDepth; i++) {
      for (auto e : nextProps) {
        cone.inPropositions.push_back(new PropositionNext(oden::copy(*e), i));
      }
    }
...
void AteamMiner::_selectDCVariables(const Template &antecedent,
  ...
  for (auto &kv : inputProps) {
    ...
    for (DelayedProposition &p : kv.second) {
      if (oden::prop2String(*kv.first) != oden::prop2String(consequent)) {
